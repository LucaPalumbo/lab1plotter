...
class Plotter(object):
    ...
    def scatter(self, x=None, y=None, sigma_y = None, sigma_x = None, label=None):
        ...
        if self.form['showLegend'] and label == None:
    ...
    def setAxisLabel(self, x_axis = None, y_axis = None):
        ...
        if x_axis == None:
            x_axis = self.form['xAxisLabel']
        if y_axis == None:
            y_axis = self.form['yAxisLabel']
        if (x_axis != '' ):
            plt.xlabel(x_axis)
        if (y_axis != '' ):
            plt.ylabel(y_axis)
    ...
    def doFit(self):
        ...
        modello = self.modelli[self.form['model']]
    ...
    def parse_all_data(self):
        ...
        self.form = json.loads(self.form)
        self.x_data = self.parse_data( self.form['xData'] )
        if type(self.x_data) != np.ndarray:
            return "error: C'è un errore nei valori dell'asse x"
        self.y_data = self.parse_data( self.form['yData'] )
        if type(self.y_data) != np.ndarray:
            return "error: C'è un errore nei valori dell'asse y"
        if self.form['showUncertainties'] == True:
            self.sigma_x = self.parse_data( self.form['ySigma'] )
            if type(self.sigma_x) != np.ndarray:
                return "error: C'è un errore negli errori dell'asse x"
            self.sigma_y = self.parse_data( self.form['xSigma'] )
            if type(self.sigma_y) != np.ndarray:
